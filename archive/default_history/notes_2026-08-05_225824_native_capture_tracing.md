...
## [2026-08-05 23:28:00] [Session ID: omx-1785926019233-oohizd] 笔记: 三项 GUI control 能力验证
- macOS capture backend 每条路径 5 秒 bounded;`sck-rs` 超时后 fallback `xcap`,同一 backend 同时只保留一个 worker。
- Terminal live smoke 证明 app-menu、窗口 crop 和 `@type-text mode:"auto"` 的 targeted-keyboard 回退均走过真实 control lane。

## [2026-08-05 22:05:39] [Session ID: omx-1785926019233-oohizd] 笔记: App 菜单查询被全局 AX 截断

### 现象
- 实机 `@ax-find:{root:"app-menu",app:"Finder",role:"AXMenuBarItem",limit:20}` 返回 `truncated:true`、`ref_count:1007` 与零匹配。

### 静态证据
- `snapshot_app_menus` 遍历全部可见应用,仅在累计元素数到达 `max_elements` 后停止。
- `build_ax_find_response_json` 在 snapshot 完成后才调用 `collect_matches`,因此 `query.process` 无法影响 capture 范围。
- 当前 parser 把 app-menu 的 `app` 写入 `query.process`,所以目标应用没有进入 backend capture request。

### 修复口径
- app-menu 的 `app` 是 capture selector,不是 AX process 文本过滤器。
- 复用现有 `resolve_unique_app_window_id` 的 exact-app 规则取得 canonical window ID,随后只取其 PID 的 `AXMenuBar`。
- 若 app 不能被现有规则唯一解析,保持 fail-closed;不能退回全局 capture 后猜测匹配。

## [2026-08-05 21:33:30] [Session ID: omx-1785926019233-oohizd] 笔记: 三项 GUI control 能力的现状与实施边界

### 已验证事实
- `@ax-find` 的 `app` 选择器当前会解析成唯一可交互窗口;macOS AX target id 也只支持 `pid:<pid>/window:<index>/path:<...>`。这使 `AXMenuBar` / `AXMenuItem` 无法成为可执行目标。
- `@screenshot` 当前只接受 `target:"display"`;`window`、`window_id` 等字段被明确拒绝。显示器截图已有统一 virtual-desktop composite 和 `os-logical` 坐标变换。
- `@type-text mode:"auto"` 已在 macOS 后端按 `AXValue -> targeted keyboard -> clipboard(opt-in)` 尝试。AXValue 不可写属于 `InvalidInput`,会进入下一层;显式 `mode:"ax-value"` 仍 fail-closed。

### 实施决定
- App 菜单复用 AX snapshot / action 链,增加 app-menu root 与稳定 target id,不新建命令。
- 窗口截图复用当前 all-display capture,在 fresh AX window rect 上做明确标注的 display-composite crop;不伪称为绕过遮挡的原生 window surface capture。
- 为现有 type-text 回退补定向回归覆盖,不重写已正确工作的路径。
