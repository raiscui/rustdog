...
## [2026-08-05 18:36:22] [Session ID: omx-1785926019233-oohizd] 任务名称: 登记 GUI control 后续候选

### 任务内容
- 将 App 级菜单 AX 支持、窗口级截图和 `@type-text` AXValue 自动回退登记为后续候选。

### 完成过程
- 查重 `LATER_PLANS.md` 和现有非鼠标语义规格,确认三项尚未独立登记。
- 保留 `@type-text mode:"auto"` 的既定回退梯子,且显式 AXValue 写入继续 fail-closed。
- 写入后发现条目未处于文件末尾,已立即移至真实末尾并完成单次出现校验。

### 总结感悟
- 后续候选必须明确为未实现项,不得被描述成当前协议行为。

## [2026-08-05 23:28:00] [Session ID: omx-1785926019233-oohizd] 任务名称: App 菜单 AX、窗口截图与文本输入回退

### 任务内容
- 实现 `root:"app-menu"` PID 定向 AX capture、`target:"window"` screenshot crop 和 `@type-text mode:"auto"` 的可恢复 AXValue 回退。
- 将 app-menu / window screenshot / text auto 三项已完成候选从 `LATER_PLANS.md` 清除。

### 完成过程
- 新增 macOS capture timeout guard,每个 backend 至多一个 worker;修复 4 个旧 `AxTreeRequest` 测试字面量的兼容字段。
- 更新 canonical skill 与协议规格,明确菜单只要求唯一 PID,窗口截图是 screen-composited crop。
- 临时 TCP daemon live smoke:7 个 Terminal 菜单项和 `AXPress`/`AXCancel` 成功;窗口 JPEG/manifest 均为 `570x399`;文本 auto 返回 `delivered_via:"targeted-keyboard"`,并由 AX 值读回。

### 验证
- 定向 nextest:screenshot 27,app-menu parser 11,capture 1,type-text actions 30,protocol 90。
- 完整二进制 nextest:680 passed,1 skipped;`cargo fmt -- --check` 与 `cargo build --package rustdog --bin rdog` 通过。
