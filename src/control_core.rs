...
pub fn execute_explicit_control_request<E: ControlActionExecutor>(
    ...
    shell: &str,
    executor: &E,
    cancel_registry: &crate::cancellation::CancelRegistry,
    ...
    match &request.command {
        ...
        ControlCommand::Record(record_request) => {
            handle_record_command(record_request, request.request_id)
        }
...
pub fn parse_and_execute_control_line<E: ControlActionExecutor>(
    ...
    match parse_control_line(line) {
        Ok(ControlParseResult::Control(request)) => {
            execute_explicit_control_request(&request, shell, executor, cancel_registry)
        }
...
fn handle_record_command(
    ...
    match with_recording_handler(|handler| handler.handle(request_id, connection, request.clone()))
    {
        ...
        None => ControlExecutionOutcome::from_response_line(render_protocol_error_response(
            ...
            4111,
            &format!(
                "{{\"schema\":\"{}\",\"error_code\":\"RECORDING_NOT_INITIALIZED\"}}",
                "rdog.record-control.v1"
            ),
        )),
    }
}
