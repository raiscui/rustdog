...
impl TextEditFixture {
    fn create(name: &str) -> Self {
        let file_path = std::env::temp_dir().join(format!(
            "rdog-window-e2e-{name}-{}-{}.txt",
            std::process::id(),
            next_free_port()
        ));
        fs::write(&file_path, b"rdog window control live e2e\n")
            .expect("TextEdit fixture file should write");
...
fn textedit_query(title_needle: &str) -> String {
    format!(
        "{{app:\"TextEdit\",title_contains:\"{}\",limit:5,include_state:true,include_recipes:true}}",
        title_needle.replace('\"', "\\\"")
    )
}

fn window_find(binary: &Path, port: u16, title_needle: &str) -> Value {
    let response = run_control_command(
        binary,
        port,
        &format!("@window-find#100:{}\n", textedit_query(title_needle)),
        Duration::from_secs(20),
    );
    successful_response_value(response, "@window-find", binary)
}

fn wait_for_window(binary: &Path, port: u16, title_needle: &str, timeout: Duration) -> Value {
    let deadline = Instant::now() + timeout;

    while Instant::now() < deadline {
        let find = window_find(binary, port, title_needle);
    ...
    panic!("window query never matched title needle: {title_needle}");
}
...
fn wait_for_window_state(
    ...
    while Instant::now() < deadline {
        let find = wait_for_window(binary, port, title_needle, Duration::from_secs(2));
    ...
    let find = window_find(binary, port, title_needle);
    panic!(
        "window never reached expected state `{predicate_label}`: {}",
        json_excerpt(&find, 4000)
    );
}

fn wait_for_no_window(binary: &Path, port: u16, title_needle: &str, timeout: Duration) -> Value {
    let deadline = Instant::now() + timeout;
    ...
    let find = window_find(binary, port, title_needle);
    panic!(
        "window should be gone after close, but query still matched: {}",
        json_excerpt(&find, 4000)
    );
}
...
fn daemon_control_lane_should_find_activate_and_close_real_hidden_minimized_occluded_window() {
    ...
    let daemon = start_live_window_daemon("textedit-window-state");
    ...
    let fixture = TextEditFixture::create("states");
    let title = fixture.title_needle.clone();
    let initial_find = wait_for_window(binary, port, &title, Duration::from_secs(20));
    let initial_candidate = first_match(&initial_find);
    let stable_window_id = window_id(initial_candidate);

    run_applescript("tell application \"Terminal\" to activate");
    let occluded_find = wait_for_window_state(binary, port, &title, "occluded", |candidate| {
    ...
    let occluded_activate = activate_window(binary, port, &stable_window_id);
    ...
    let occluded_recovered = wait_for_window_state(
        binary,
        port,
        &title,
    ...
    let minimized_find = wait_for_window_state(binary, port, &title, "minimized", |candidate| {
    ...
    let minimized_activate = activate_window(binary, port, &stable_window_id);
    assert_activate_report(
        &minimized_activate,
        &stable_window_id,
    ...
    let minimized_recovered = wait_for_window_state(
        binary,
        port,
        &title,
    ...
    let hidden_find = wait_for_window_state(binary, port, &title, "hidden", |candidate| {
    ...
    let hidden_activate = activate_window(binary, port, &stable_window_id);
    assert_activate_report(
        &hidden_activate,
        &stable_window_id,
    ...
    let hidden_recovered = wait_for_window_state(
        binary,
        port,
        &title,
    ...
    let close_report = close_window(binary, port, &stable_window_id);
    ...
    assert_eq!(
        ...
        Some(stable_window_id.as_str())
    );

    let closed_find = wait_for_no_window(binary, port, &title, Duration::from_secs(15));
